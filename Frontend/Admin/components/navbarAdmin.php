<div class="nav">
    <div class="logo">
        <i class='bx bxl-codepen'></i>
        <a href="#">XABER</a>
    </div>
    <div class="nav-links">
        <a href="indexAdmin.php?page=dashboard">Home</a>
        <div class="dropdown">
            <a href="indexAdmin.php?page=violance">Violance</a>
            <div class="dropdown-menu">
                <a href="indexAdmin.php?page=violance-sub1">Sub-link 1</a>
                <a href="indexAdmin.php?page=violance-sub2">Sub-link 2</a>
            </div>
        </div>
        <a href="indexAdmin.php?page=Attendance">Attendance</a>
        <a href="indexAdmin.php?page=Services">Services</a>
        <a href="indexAdmin.php?page=Rules">Rules</a>
    </div>
    <div class="right-section">
        <div class="profile">
            <div class="info">
                <img src="img/Profile.JPG">
                <div>
                    <a href="#">Erwan Majid</a>
                    <p>Lecturer</p>
                </div>
            </div>
            <i class='bx bx-chevron-down'></i>
        </div>
    </div>
</div>
