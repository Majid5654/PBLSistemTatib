...
<body>
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#forgot-password-form').on('submit', function (e) {
                ...
                $.ajax({
            url: 'http://localhost/PBLTatib/PBLSistemTatib/Backend/forgotPassword.php', 
            type: 'POST',
            data: { 'forgot-email': email }, // Data yang dikirim ke server
            dataType: 'json', // Tipe respons yang diharapkan (JSON)
            ...
            error: function (xhr, status, error) {
                ...
                console.log(xhr.responseText); // Tampilkan respons server di konso
                    }
                });
            });
        });
    ...
    $('#login-form').on('submit', function (e) {
        ...
        $.ajax({
            url: '../Backend/ProcessLogin.php', // URL ke file PHP
            type: 'POST',
            data: { email: email, password: password }, // Data yang dikirim
            dataType: 'json', // Respons yang diharapkan
            success: function (response) {
                if (response.success) {
                    // Jika sukses, arahkan ke halaman sesuai level
            ...
            error: function (xhr, status, error) {
                // Tampilkan pesan error jika terjadi masalah di server
                errorMessage.text('An unexpected error occurred. Please try again.').show();
                console.log(xhr.responseText); // Debugging di konsol
            }
        });
    });
});
...
<script>
    $(document).ready(function () {
        // Ambil pesan error login dari session PHP
        const loginErrorMessage = '<?php echo $_SESSION["login_error"] ?? ""; ?>';

        // Jika ada pesan error, tampilkan di elemen error
        if (loginErrorMessage) {
            $('#login-error-message').text(loginErrorMessage).show();
